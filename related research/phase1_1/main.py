...
def main():
    ...
    forums = forums.append(collect_publications(papers, 'journal', ForumType.JOURNAL))
    forums = forums.append(collect_publications(papers, 'booktitle', ForumType.CONFERENCE))
    ...
    forums.to_csv(os.path.join('phase1_1', 'publications.csv'), columns=['name', 'type', 'refs'])


def collect_publications(papers: pandas.DataFrame, column_name: str, forum_type: ForumType):
    forums = pandas.DataFrame()
    for name in papers['journal'].drop_duplicates():
